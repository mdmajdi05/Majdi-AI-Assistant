...
router.post("/ask", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Check if TTS failed
      if (generatedPath === "TTS_FAILED") {
        console.log("TTS failed, returning text response only");
    ...
    console.error("Error in Majdi route:", err);
    const fallback = "Mujhe abhi voice generate karne mein dikkat aa rahi hai. Kripya thodi der mein try karo.";
    res.status(500).json({ error: err.message || "Server error", reply: fallback });
  }
});